...
BuildLog="$ScriptsLog.$PhoneName.AOSPOMSM.log";
...
# Compilation Script
cd $ScriptsDir;
source ./android_auto_huashan_aospoms60.sh "$BuildMode" 2>&1 | tee "$BuildLog";
...
if [ ! -z "$BuildSuccess" ]; then
  PushBulletComment="AOSP-OMS M ROM for $PhoneName ready";
else
  PushBulletComment="AOSP-OMS M ROM for $PhoneName failed";
fi;
